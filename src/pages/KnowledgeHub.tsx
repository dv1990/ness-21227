...
const KnowledgeHub = () => {
  ...
  return (
    <Layout>
      {/* Hero Section */}
      <section className="relative pt-20 sm:pt-32 pb-16 sm:pb-24 bg-charcoal overflow-hidden">
        {/* Gradient Orb Field */}
        ...
        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          ...
          >
            <p className="text-caption uppercase tracking-[0.2em] sm:tracking-[0.25em] text-pearl/60 mb-4 sm:mb-6">
              Resources
            </p>
            <h1 className="text-3xl sm:text-display-medium md:text-display-large font-light text-pearl mb-6 sm:mb-8 tracking-tight">
              Knowledge Hub
            </h1>
            <p className="text-body-large text-pearl/50 max-w-xl mx-auto leading-relaxed">
      ...
      {/* Home Users Section */}
      <section ref={sectionRef} className="py-16 sm:py-24 md:py-32 bg-graphite relative overflow-hidden">
        {/* Dot pattern overlay */}
        <div
            ...
            {homeUserCards.map((card, index) => (
              <Link
                ...
                className={`group relative overflow-hidden rounded-3xl transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]
                  ...
                  ${card.accent
                    ? 'bg-pearl/[0.03] border border-energy/20 backdrop-blur-sm'
                    : 'bg-pearl/[0.03] border border-pearl/10 backdrop-blur-sm'}
              ...
              >
                {/* Specular highlight */}
                <div className="absolute inset-0 bg-gradient-to-br from-pearl/8 via-transparent to-transparent opacity-60 group-hover:opacity-80 transition-opacity duration-500 pointer-events-none" />
                ...
                <div className={`relative z-10 p-6 sm:p-8 md:p-14 h-full flex flex-col ${card.featured ? 'min-h-[280px] sm:min-h-[320px] md:min-h-[420px]' : 'min-h-[200px] sm:min-h-[220px]'}`}>
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
                    <div className={`p-3 sm:p-4 rounded-xl sm:rounded-2xl transition-all duration-500 group-hover:scale-110 ${
                      card.accent
                        ...
                        : 'bg-pearl/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)]'
                    }`}>
                      <card.icon className={`w-5 h-5 sm:w-7 sm:h-7 ${card.accent ? 'text-energy' : 'text-pearl'}`} />
                    </div>
                    {card.secondaryIcon && (
                      <div className={`p-3 sm:p-4 rounded-xl sm:rounded-2xl transition-all duration-500 group-hover:scale-110 ${
                        card.accent
                        ...
                        <card.secondaryIcon className={`w-5 h-5 sm:w-7 sm:h-7 ${card.accent ? 'text-energy' : 'text-pearl'}`} />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm sm:text-body-large text-pearl/50 leading-relaxed">
                      {card.hook}
                    </p>
                  </div>
      ...
      <section className="py-16 sm:py-24 bg-charcoal relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          ...
          >
            <p className="text-sm sm:text-body-large text-pearl/50 mb-6 sm:mb-8">
              Can't find what you're looking for?
            </p>
            <Link
              to="/contact"
              className="inline-flex items-center justify-center gap-2 sm:gap-3 px-8 sm:px-10 py-4 sm:py-5 bg-energy text-charcoal rounded-full font-semibold min-h-[48px]
                transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]
                hover:bg-energy-bright hover:scale-105 hover:shadow-glass-hover active:scale-95"
            >
              Contact Support
