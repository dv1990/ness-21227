...
export const PWAInstallPrompt = () => {
  ...
  useEffect(() => {
    ...
    // Check if user previously dismissed — read once, synchronously but outside timer
    ...
    const handleBeforeInstallPrompt = (e: Event) => {
      ...
      // Show prompt after 5 seconds, with proper cleanup
      if (!wasDismissed) {
    ...
    return () => {
      ...
      // Clean up timer on unmount to prevent memory leak / state update on unmounted component
      if (timerRef.current) {
  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      // Installation failed silently — no user action needed
    }
  };
